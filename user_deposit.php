...
if (isset($_POST['deposit'])) {
    if ($user_id == '') {
        ...
        if ($deposit <= 0) {
            $error_message = "Invalid deposit amount please enter a valid amount.";
        } elseif ($deposit % 20 !== 0) {
            ...
            if ($select_balance->rowCount() > 0) {
                $update_deposit = $conn->prepare("UPDATE wallet SET deposit = ?, balance = ?, withdraw = 0 WHERE user_id = ?");
                $update_deposit->execute([$deposit, $new_balance, $user_id]);
            } else {
                $insert_deposit = $conn->prepare("INSERT INTO wallet (user_id, deposit, balance, withdraw, name) VALUES (?, ?, ?, ?, ?)");
                $insert_deposit->execute([$user_id, $deposit, $new_balance, 0, $name]);
            }

            $insert_transaction = $conn->prepare("INSERT INTO transactions (transaction_id, user_id, name, balance, deposit, withdraw, method, date_transaction) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $insert_transaction->execute([$transaction_id, $user_id, $name, $new_balance, $deposit, 0, $transaction_method, $date_transaction]);

            $success_message = "Successfully deposited in your wallet";
        }
    }
}
...
<div class="container">
    <div class="withdraw-form">
        <h1 class="text-center">Deposit Form</h1>
        <?php 
        ...
        <form method="POST" action="">
            <div class="form-group">
                <label for="deposit" style="margin-top: 30px;">Deposit:</label>
                <input type="number" class="form-control" id="deposit" name="deposit" required>
            </div>
            <button type="submit" class="btn btn-primary mt-1">Deposit</button>
        </form>
    </div>
</div>
